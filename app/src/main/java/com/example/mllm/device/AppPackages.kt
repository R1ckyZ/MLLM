...
object AppPackages {
    private val defaultPackages: Map<String, String> =
        linkedMapOf(
            "MLLM" to "com.example.mllm",
    ...
    private val packageMap: LinkedHashMap<String, String> = LinkedHashMap(defaultPackages)

    fun updateFromText(text: String) {
        val parsed = parseText(text)
        packageMap.clear()
        if (parsed.isNotEmpty()) {
            packageMap.putAll(parsed)
        } else {
            packageMap.putAll(defaultPackages)
        }
    }

    fun toText(): String {
        return packageMap.entries.joinToString("\n") { "${it.key}=${it.value}" }
    }

    fun packageFor(appName: String): String? = packageMap[appName]

    fun appNameForPackage(packageName: String): String? {
        for ((name, pkg) in packageMap) {
            if (pkg == packageName) {
                return name
            }
        }
    ...
    private fun parseText(text: String): LinkedHashMap<String, String> {
